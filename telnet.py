...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if self._play_context.check_mode:
            ...
            timeout = self._task.args.get('timeout', self._play_context.timeout)
            port = self._task.args.get('port', self._play_context.port)
            display.vvv("%s %s %s %s %s" %(host,  user, password, timeout, port))

            # FIXME, default to play_context?
            # port = self._task.args.get('port', '23')
            # with timeout = self._task.args.get('timeout', self._play_context.timeout), the default value is 10
